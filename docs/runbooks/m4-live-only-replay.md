...
Snapshot V2 provenance is execution-bound. The production entry retains its
accepted policy-mode switch, but its V2 branch reaches only the facade in
`cmd/dota2-ob/broadcast_runtime.go`. That facade imports only
`internal/snapshotv2/compiled/product`; it cannot import the current insight,
policy, presentation, or recovery implementations. The compiled package is
...
both executable bytes and EngineBuild identity. Current V3 stays on its
separately fingerprinted runtime. The accepted base regression continues to
...
go test -count=2 ./cmd/dota2-ob ./internal/integration/m4
go test -count=2 ./internal/snapshotv2 ./cmd/dota2-ob -run 'TestCompiledV2|TestProductionV2|TestSemanticMutation|TestEmbeddedReference|TestSnapshotV2AcceptedCanonicalBytesAndLineageRemainPinned|TestBroadcastRuntimeV3DoesNotRewindCausalBaselineAndRestartsAtNewest|TestBroadcastRuntimeV3QueueSaturationLatchesHealthHideAndRecovers|TestM4'
go test -count=1 ./...
