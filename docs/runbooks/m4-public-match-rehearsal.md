...
Before launching those owned components, the command requires exactly one
already-running `dota2` process. It never launches or controls Dota. It binds
the accepted low-risk live identity (PID, `comm`, executable content and path
hashes, and process start ticks), then rechecks it at every retained raw
sequence and at capture/shutdown terminal transitions. Exit, PID reuse, path or
binary replacement, and start-tick drift fail closed.
...
re-executes all eight family-specific production eligibility sites with typed unavailable
...
A canonical refused preflight is also terminal: `rehearsal-attempt` launches
neither the product nor OBS and seals `preflight_refused`. Verification checks
the immutable failed registry and a fresh listener observation without
requiring a deliberately failed remote/PR check to later become passing. The
same verified-terminal hash and wrong-token rules govern cleanup.
