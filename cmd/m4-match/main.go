...
func run(args []string) int {
	...
	case "rehearsal-attempt":
		...
		repoRoot := flags.String("repo-root", repo, "repository root")
		raw := flags.String("raw-session", "", "retained RawRecordV3 JSONL")
		sessionID := flags.String("session-id", "", "exact retained session identity")
		matchID := flags.String("match-id", "", "stable nonzero public match ID")
		preview := flags.Bool("preview-accepted", true, "operator preview accepted")
		entryBeforeZero := flags.Bool("entry-before-zero", true, "entry occurred before game clock 0:00")
		lateJoin := flags.Bool("late-join", false, "attempt joined after 0:00")
		processStable := flags.Bool("process-stable", true, "Dota process identity remained stable")
		processReplaced := flags.Bool("process-replaced", false, "Dota process identity was replaced")
		processExecutable := flags.String("process-executable-sha256", "", "bound Dota executable SHA-256")
		processStartTicks := flags.Uint64("process-start-ticks", 0, "bound Dota process start tick")
		continuous := flags.Bool("plausible-continuous-frames", true, "raw frames were plausible and continuous")
		postGame := flags.Bool("normal-post-game", true, "normal winner/post-game observed")
		obsFinalized := flags.Bool("obs-finalized", true, "OBS recording finalized completely")
		rawReconciled := flags.Bool("raw-reconciled", true, "raw-first population reconciled")
		clocksValid := flags.Bool("clocks-valid", true, "clock evidence reconciled")
		resourcesSampled := flags.Bool("resources-sampled", true, "five-second resource evidence complete")
		operatorActions := flags.Uint("operator-actions", 4, "completed prescribed operator action count")
		failClosed := flags.Bool("fail-closed-within-2s", true, "claim-free fail-closed deadline met")
		confined := flags.Bool("confinement-verified", true, "evidence confinement verified")
		cleanExit := flags.Bool("clean-exit", true, "attempt exited cleanly after recovery verification")
		if flags.Parse(args[1:]) != nil || *root == "" || *sessionID == "" || *operatorActions > 255 {
			return 2
		}
		request := m4match.RehearsalAttemptRequestV1{
			SchemaVersion: m4match.RehearsalAttemptSchemaVersion, SessionID: *sessionID, MatchID: *matchID,
			PreviewAccepted: *preview, EntryBeforeZero: *entryBeforeZero, LateJoin: *lateJoin,
			ProcessStable: *processStable, ProcessReplaced: *processReplaced,
			ProcessExecutableSHA256: *processExecutable, ProcessStartTicks: *processStartTicks,
			PlausibleContinuous: *continuous, NormalPostGame: *postGame, OBSFinalized: *obsFinalized,
			RawReconciled: *rawReconciled, ClocksValid: *clocksValid, ResourcesSampled: *resourcesSampled,
			OperatorActions: uint8(*operatorActions), FailClosedWithin2S: *failClosed,
			ConfinementVerified: *confined, CleanExit: *cleanExit,
		}
		result, err := m4match.RehearsalAttempt(context.Background(), m4match.RehearsalAttemptConfig{DataRoot: *root, RepoRoot: *repoRoot, RawPath: *raw, Request: request})
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return 1
		}
