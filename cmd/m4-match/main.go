...
func run(args []string) int {
	...
	case "rehearsal-preflight":
		...
		root := flags.String("data-root", "", "fresh absolute rehearsal evidence root outside the repository")
	...
	case "rehearsal-cleanup":
		...
		repoRoot := flags.String("repo-root", repo, "repository root")
		confirm := flags.String("confirm-index-sha256", "", "required rehearsal evidence index SHA-256")
...
func usage() {
	fmt.Fprintln(os.Stderr, "usage: m4-match {preflight|live|verify|cleanup|rehearsal-preflight|rehearsal-verify|rehearsal-cleanup} [flags]")
}
