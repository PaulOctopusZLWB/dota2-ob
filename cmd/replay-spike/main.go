...
const opendotaMatchURL = "https://api.opendota.com/api/matches/%s"
...
func cmdParse(args []string) {
	...
	fmt.Printf("\n[second pass] hash=%s json_bytes=%d elapsed=%.3fs peak_heap=%dMiB\n",
		pr2.Hash, len(b2), pr2.Metrics.ElapsedSec, pr2.Metrics.PeakHeapMiB)
...
func printOne(path, out string) string {
	...
	fmt.Printf("elapsed_sec=%.3f peak_heap_mib=%d num_gc=%d goroutines=%d raw_bytes=%d\n",
		pr.Metrics.ElapsedSec, pr.Metrics.PeakHeapMiB, pr.Metrics.NumGC, pr.Metrics.Goroutines, pr.Metrics.RawDecompressed)
...
func cmdAcquire(args []string) {
	...
	if meta.ReplayURL == "" {
		fail("no replay_url for match %s (OpenDota may have purged it)", matchID)
	}

	bz2Path := filepath.Join(replaysDir, matchID+".dem.bz2")
	demPath := filepath.Join(replaysDir, matchID+".dem")
	dlAt := time.Now().UTC()
	if err := download(meta.ReplayURL, bz2Path); err != nil {
		fail("download: %v", err)
	}
	...
	dBytes, dSHA, compression, err := zstdDecompress(bz2Path, demPath)
	if err != nil {
	...
	rec := acquireRecord{
		...
		ReplayURL: meta.ReplayURL, Cluster: meta.Cluster, PatchVersion: meta.PatchVersion,
	...
	fmt.Printf("  league=%q tier=%q radiant=%s dire=%s patch=%d duration=%ds players=%d\n",
		meta.LeagueName, meta.LeagueTier, meta.RadiantName, meta.DireName, meta.PatchVersion, meta.Duration, meta.PlayerCount)
...
func fetchOpenDotaMatch(matchID string) (matchMeta, error) {
	resp, err := http.Get(fmt.Sprintf(opendotaMatchURL, matchID))
	if err != nil {
	...
	return matchMeta{
		ReplayURL: raw.ReplayURL, Cluster: raw.Cluster, PatchVersion: raw.Version,
...
func zstdDecompress(src, dst string) (int64, string, string, error) {
	in, err := os.Open(src)
	if err != nil {
		return 0, "", "", err
	}
