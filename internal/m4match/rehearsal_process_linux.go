...
func readRehearsalOwnedProcessIdentity(pid int) (RehearsalOwnedProcessIdentityV1, error) {
	base := filepath.Join("/proc", strconv.Itoa(pid))
	commBytes, err := os.ReadFile(filepath.Join(base, "comm"))
	if err != nil || strings.TrimSpace(string(commBytes)) == "" {
		return RehearsalOwnedProcessIdentityV1{}, errors.New("owned_process_identity_unavailable")
	}
	...
	startAfter, startErr := processStartTicks(filepath.Join(base, "stat"))
	if hashErr != nil || postErr != nil || linkErr != nil || startErr != nil || read != pre.Size() || !os.SameFile(pre, post) || pre.Size() != post.Size() || pathBefore != pathAfter || startBefore != startAfter {
		return RehearsalOwnedProcessIdentityV1{}, errors.New("owned_process_identity_changed_during_acquisition")
	}
	pathHash := sha256.Sum256([]byte(pathBefore))
	return RehearsalOwnedProcessIdentityV1{
		SchemaVersion: "rehearsal_owned_process_identity.v1", PID: pid, Comm: comm, ExecutablePath: pathBefore,
		ExecutablePathSHA256: hex.EncodeToString(pathHash[:]), ExecutableSHA256: hex.EncodeToString(hash.Sum(nil)),
		ProcessStartTicks: startBefore, ExecutableDevice: uint64(preStat.Dev), ExecutableInode: preStat.Ino,
	}, nil
}

func sameRehearsalOwnedProcessIdentity(left, right RehearsalOwnedProcessIdentityV1) bool {
	return left == right
}
