...
func loadRehearsalCoverageFrames(path string) ([]CoverageFrameV1, string, error) {
	payload, err := os.ReadFile(path)
	if err != nil || len(payload) == 0 || payload[len(payload)-1] != '\n' {
		return nil, "", errors.New("rehearsal raw session is absent or unterminated")
	}
	scanner := bufio.NewScanner(bytes.NewReader(payload))
	scanner.Buffer(make([]byte, 64<<10), 16<<20)
	var frames []CoverageFrameV1
	for scanner.Scan() {
		...
		if err := json.Unmarshal(line, &record); err != nil || record.SchemaVersion != 3 || record.Sequence != uint64(len(frames)+1) || record.RawEncoding != "base64_std" {
			return nil, "", errors.New("rehearsal raw record identity is invalid")
		}
		raw, decodeErr := base64.StdEncoding.Strict().DecodeString(record.RawBase64)
		if decodeErr != nil || len(raw) != record.RawByteLength || payloadSHA(raw) != record.RawPayloadSHA256 || !json.Valid(raw) {
			return nil, "", errors.New("rehearsal raw payload identity mismatch")
		}
		frames = append(frames, CoverageFrameV1{Sequence: record.Sequence, RawRecordSHA256: payloadSHA(line), RawRecord: raw, IdentityRecord: line})
	}
	if err := scanner.Err(); err != nil || len(frames) == 0 {
		return nil, "", errors.New("rehearsal raw session population is invalid")
	}
	return frames, payloadSHA(payload), nil
}

func generateRehearsalCoverage(root, repo, sessionID, candidateCommit string) (RehearsalCoverageBindingV1, error) {
	...
	if err != nil {
		return RehearsalCoverageBindingV1{}, err
	}
	rehearsalPath := filepath.Join(root, "data/sessions", sessionID, "raw.jsonl")
	rehearsal, rehearsalSHA, err := loadRehearsalCoverageFrames(rehearsalPath)
	if err != nil {
		return RehearsalCoverageBindingV1{}, err
	}
	rootPayload, _ := canonical(struct {
		Domain, CandidateCommit, BaselineSHA256, RehearsalSHA256, Algorithm string
	}{rehearsalRootDomain, candidateCommit, baselineSHA, rehearsalSHA, CoverageNormalizationVersion})
	rootSHA := payloadSHA(rootPayload)
	delta, err := GenerateFieldCoverageDelta(baseline, rehearsal, baselineSHA, rehearsalSHA, rootSHA)
	if err != nil {
		return RehearsalCoverageBindingV1{}, err
	}
	payload, _ := canonical(delta)
	relative := "evidence/canonical/field-coverage-delta.json"
	if err := writePrivate(filepath.Join(root, filepath.FromSlash(relative)), payload); err != nil {
		return RehearsalCoverageBindingV1{}, err
	}
	return RehearsalCoverageBindingV1{ArtifactPath: relative, SHA256: payloadSHA(payload), RootSHA256: rootSHA}, nil
}
