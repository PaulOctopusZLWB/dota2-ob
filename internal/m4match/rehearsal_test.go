...
func testValidatedCaptureBinding(raw scannedRehearsalRaw) rehearsalCaptureBinding {
	...
	for sequence := uint64(1); sequence <= raw.Manifest.Accepted; sequence++ {
		h.Process = append(h.Process, RehearsalProcessObservationV1{SourceSequence: sequence, PID: 123, Comm: "dota2", ExecutablePathSHA256: strings.Repeat("a", 64), ExecutableSHA256: strings.Repeat("5", 64), StartTicks: 9, State: "running"})
	}
	if raw.Manifest.Accepted > 0 {
		h.TerminalProcess = []RehearsalProcessObservationV1{
			{SourceSequence: raw.Manifest.Accepted, PID: 123, Comm: "dota2", ExecutablePathSHA256: strings.Repeat("a", 64), ExecutableSHA256: strings.Repeat("5", 64), StartTicks: 9, State: "terminal_capture"},
			{SourceSequence: raw.Manifest.Accepted, PID: 123, Comm: "dota2", ExecutablePathSHA256: strings.Repeat("a", 64), ExecutableSHA256: strings.Repeat("5", 64), StartTicks: 9, State: "terminal_shutdown"},
		}
...
func TestExecutableProducerOwnsAndSealsPhysicalArtifacts(t *testing.T) {
	...
	rehearsalRunComponents = func(_ context.Context, runRoot, _ string, runOwner RehearsalCaptureOwnerV1, _ string) (rehearsalComponentResult, error) {
		...
		dota := LiveIdentity{DotaPID: 123, DotaExecutableSHA256: strings.Repeat("6", 64), DotaProcessStartTicks: 77}
		process := make([]RehearsalProcessObservationV1, 0, 3)
		for sequence := uint64(1); sequence <= 3; sequence++ {
			process = append(process, RehearsalProcessObservationV1{SourceSequence: sequence, PID: dota.DotaPID, Comm: "dota2", ExecutablePathSHA256: strings.Repeat("7", 64), ExecutableSHA256: dota.DotaExecutableSHA256, StartTicks: dota.DotaProcessStartTicks, State: "running"})
		}
		terminal := []RehearsalProcessObservationV1{
			{SourceSequence: 3, PID: dota.DotaPID, Comm: "dota2", ExecutablePathSHA256: strings.Repeat("7", 64), ExecutableSHA256: dota.DotaExecutableSHA256, StartTicks: dota.DotaProcessStartTicks, State: "terminal_capture"},
			{SourceSequence: 3, PID: dota.DotaPID, Comm: "dota2", ExecutablePathSHA256: strings.Repeat("7", 64), ExecutableSHA256: dota.DotaExecutableSHA256, StartTicks: dota.DotaProcessStartTicks, State: "terminal_shutdown"},
		}
		return rehearsalComponentResult{ProductIdentity: identity, DotaIdentity: dota, DotaProcess: process, DotaTerminal: terminal, Validation: validation, RecoveryFirst: proof, RecoverySecond: second, ProductClean: true, OBSClean: true}, nil
	}
...
func TestRehearsalDotaIdentityBindsPIDExecutablePathAndStartTicks(t *testing.T) {
	procRoot := t.TempDir()
	executable := filepath.Join(t.TempDir(), "dota2-bin")
	if err := os.WriteFile(executable, []byte("accepted-dota-binary"), 0o700); err != nil {
		t.Fatal(err)
	}
	writeProcess := func(pid int, start uint64, target string) {
		directory := filepath.Join(procRoot, fmt.Sprint(pid))
		...
		if err := os.WriteFile(filepath.Join(directory, "stat"), []byte(fmt.Sprintf("%d (dota2) %s\n", pid, strings.Join(fields, " "))), 0o600); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(directory, "comm"), []byte("dota2\n"), 0o600); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(directory, "cmdline"), []byte("dota2\x00"), 0o600); err != nil {
			t.Fatal(err)
		}
		_ = os.Remove(filepath.Join(directory, "exe"))
		if err := os.Symlink(target, filepath.Join(directory, "exe")); err != nil {
			t.Fatal(err)
		}
	}
	writeProcess(4242, 101, executable)
	identity, err := acquireRehearsalDotaProcess(procRoot)
	if err != nil {
		t.Fatal(err)
	}
	first, err := observeRehearsalDotaProcess(procRoot, identity, 1, "running")
	if err != nil || first.PID != 4242 || first.Comm != "dota2" || !validSHA256(first.ExecutablePathSHA256) {
		t.Fatalf("Dota identity not bound: %#v %v", first, err)
	}
	writeProcess(4242, 102, executable)
	if _, err := observeRehearsalDotaProcess(procRoot, identity, 2, "running"); err == nil {
		t.Fatal("PID reuse/start-tick drift accepted")
	}
	writeProcess(4242, 101, executable)
	replacement := filepath.Join(t.TempDir(), "dota2-replacement")
	if err := os.WriteFile(replacement, []byte("replacement"), 0o700); err != nil {
		t.Fatal(err)
	}
	writeProcess(4242, 101, replacement)
	if _, err := observeRehearsalDotaProcess(procRoot, identity, 2, "running"); err == nil {
		t.Fatal("Dota executable/path drift accepted")
	}
	writeProcess(4243, 103, executable)
	if _, err := acquireRehearsalDotaProcess(procRoot); err == nil {
		t.Fatal("ambiguous Dota process selection accepted")
	}
}

func TestRemoteAndDraftRefusalsSealVerifyAndCleanup(t *testing.T) {
	repo, err := filepath.Abs(filepath.Join("..", ".."))
	if err != nil {
		t.Fatal(err)
	}
	head, tree := strings.Repeat("a", 40), strings.Repeat("b", 40)
	command := func(_ context.Context, _ string, name string, args ...string) (string, error) {
		joined := name + " " + strings.Join(args, " ")
		switch joined {
		case "git rev-parse HEAD":
			return head, nil
		case "git show -s --format=%P HEAD":
		...
		default:
			if strings.Contains(joined, "ls-remote") {
				return "", errors.New("sealed remote refusal")
			}
			return "", fmt.Errorf("unexpected test command: %s", joined)
		}
	}
	runClass := func(label string, draftFailed bool) string {
		root := filepath.Join("/var/tmp", fmt.Sprintf("dot84-refused-%d-%s", os.Getpid(), label))
		_ = os.RemoveAll(root)
		readPR := func(context.Context, string) (rehearsalPR, error) {
			if draftFailed {
				return rehearsalPR{}, errors.New("sealed draft refusal")
			}
			return rehearsalPR{Number: 26, URL: "https://github.com/PaulOctopusZLWB/dota2-ob/pull/26", HeadRefOID: head, IsDraft: true}, nil
		}
		readiness, err := RehearsalPreflight(context.Background(), RehearsalPreflightConfig{DataRoot: root, RepoRoot: repo, runText: command, readPR: readPR})
		if err != nil {
			t.Fatal(err)
		}
		if readiness.Ready || readiness.ConsoleState != rehearsalRefusedConsoleState || !containsString(readiness.Failures, "remote_branch_head") || draftFailed != containsString(readiness.Failures, "draft_pr_head") {
			t.Fatalf("wrong refused registry: %#v", readiness)
		}
		...
		if err != nil || receipt.Outcome != "rehearsal_failed" || receipt.FailureCode != "preflight_refused" {
			t.Fatalf("refusal did not seal: %#v %v", receipt, err)
		}
		verified, err := VerifyRehearsalTerminal(context.Background(), root, repo)
		if err != nil || verified != receipt {
			t.Fatalf("refusal did not independently verify: %#v %v", verified, err)
		}
		terminalPath := filepath.Join(root, "evidence/canonical/terminal.json")
		terminalPayload, err := os.ReadFile(terminalPath)
		if err != nil {
			t.Fatal(err)
		}
		tampered := append([]byte(nil), terminalPayload...)
		tampered[len(tampered)/2] ^= 1
		if err := os.WriteFile(terminalPath, tampered, 0o600); err != nil {
			t.Fatal(err)
		}
		if _, err := VerifyRehearsalTerminal(context.Background(), root, repo); err == nil {
			t.Fatal("tampered refused terminal verified")
		}
		if err := os.WriteFile(terminalPath, terminalPayload, 0o600); err != nil {
			t.Fatal(err)
		}
		if _, err := VerifyRehearsalTerminal(context.Background(), root, repo); err != nil {
			t.Fatalf("restored refused terminal did not verify: %v", err)
		}
		if err := CleanupRehearsal(root, repo, strings.Repeat("0", 64)); err == nil {
			t.Fatal("wrong token removed refused root")
		}
		if _, err := os.Stat(root); err != nil {
			t.Fatal("wrong token did not preserve refused root")
		}
		if err := CleanupRehearsal(root, repo, receipt.TerminalSHA256); err != nil {
			t.Fatal(err)
		}
		if _, err := os.Stat(root); !os.IsNotExist(err) {
			t.Fatal("verified refused root still exists")
		}
		return receipt.TerminalSHA256
	}
	first := runClass("remote-a", false)
	second := runClass("remote-b", false)
	if first != second {
		t.Fatalf("fresh remote refusals differ: %s != %s", first, second)
	}
	runClass("remote-draft", true)
}
