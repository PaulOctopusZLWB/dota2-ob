...
func deriveProducerHarness(root string, owner RehearsalCaptureOwnerV1, manifest RehearsalRawManifestV1, result rehearsalComponentResult) (RehearsalHarnessEvidenceV1, error) {
	if !result.ProductClean || !result.OBSClean || result.ProductIdentity.StartTicks == 0 || result.ProductIdentity.ExecutableSHA256 == "" {
		return RehearsalHarnessEvidenceV1{}, errors.New("producer processes did not close cleanly")
	}
	harness := RehearsalHarnessEvidenceV1{SchemaVersion: "public_match_rehearsal_harness_evidence.v2", SessionID: owner.SessionID, RawSHA256: manifest.RawSHA256, PartialRecordings: 0}
	for sequence := uint64(1); sequence <= manifest.Accepted; sequence++ {
		harness.Process = append(harness.Process, RehearsalProcessObservationV1{SourceSequence: sequence, ExecutableSHA256: result.ProductIdentity.ExecutableSHA256, StartTicks: result.ProductIdentity.StartTicks, State: "running"})
	}
