...
const (
	...
	ExpectedRehearsalBranch         = "agent/dota2-fullstack-engineer/DOT-84-rehearsal-evidence-v3"
	...
	MaxRehearsalRawLineBytes        = (((10 << 20) + 2) / 3 * 4) + (4 << 10)
...
func RehearsalPreflight(ctx context.Context, config RehearsalPreflightConfig) (RehearsalReadinessV1, error) {
	...
	owner := RehearsalCaptureOwnerV1{
		...
		RawRelativePath:     "capture/" + sessionID + "/raw.jsonl",
		HarnessRelativePath: "capture/" + sessionID + "/harness-evidence.json",
		SealRelativePath:    "capture/" + sessionID + "/capture-seal.json",
	}
	ownerPayload, err := canonical(owner)
	if err != nil {
		return RehearsalReadinessV1{}, err
	}
	store, err := session.NewStore(filepath.Join(lease.abs, "capture"), session.WithSessionID(sessionID))
	if err != nil {
		return RehearsalReadinessV1{}, err
	}
...
func verifyRehearsalPreflight(ctx context.Context, root, repo string, listen func(string) (io.Closer, error)) (RehearsalReadinessV1, error) {
	...
	if strictCanonicalRehearsalJSON(ownerPayload, &owner) != nil || owner.SchemaVersion != "public_match_rehearsal_capture_owner.v1" || owner.CandidateCommit != evidence.CandidateCommit || owner.SessionID == "" || owner.RawRelativePath != "capture/"+owner.SessionID+"/raw.jsonl" || owner.HarnessRelativePath != "capture/"+owner.SessionID+"/harness-evidence.json" || owner.SealRelativePath != "capture/"+owner.SessionID+"/capture-seal.json" {
...
func scanRehearsalRaw(payload []byte, sessionID string) scannedRehearsalRaw {
	...
	return scanRehearsalReader(bytes.NewReader(payload), int64(len(payload)), sessionID)
}
...
func scanRehearsalRawPath(path, sessionID string) scannedRehearsalRaw {
	...
	if info.Size() > MaxRehearsalRawBytes {
		return limitedRawManifest(sessionID, info.Size(), "raw_total_limit")
	}
	return scanRehearsalReader(file, info.Size(), sessionID)
}
...
func scanRehearsalReader(input io.Reader, size int64, sessionID string) scannedRehearsalRaw {
	hash := sha256.New()
	reader := bufio.NewReaderSize(io.TeeReader(input, hash), MaxRehearsalRawLineBytes+1)
	result := scannedRehearsalRaw{}
	result.Manifest = RehearsalRawManifestV1{SchemaVersion: RehearsalRawManifestSchema, SessionID: sessionID, RawBytes: size, Records: []RehearsalRawIdentityV1{}}
	result.Manifest.RawBytes = size
	if size == 0 {
		result.Manifest.RawSHA256 = payloadSHA(nil)
		return result
	}
	for {
	...
	if result.Manifest.FailureCode == "" {
		...
		}{result.Manifest.FailureCode, size, result.Manifest.Accepted})
...
func (collector *suppressionCollector) HistoricalUnavailable(event insight.HistoricalUnavailableExecution) error {
	if event.Reason != "invalid_history_binding" {
...
func evaluateRehearsalProduction(raw scannedRehearsalRaw) (RehearsalSuppressionEvidenceV1, string) {
	...
	for _, record := range raw.Records {
		...
		candidates, branchErr := insight.EvaluateLiveOnlyProductionBranches(insight.LiveOnlyInput{
		...
		}, insight.DefaultConfig(), collector)
		if branchErr != nil {
			return result, "production_evaluation_failure"
		}
...
func loadRehearsalCapture(root string, readiness RehearsalReadinessV1, request RehearsalAttemptRequestV1) (rehearsalCaptureBinding, scannedRehearsalRaw) {
	...
	if binding.Owner.CandidateCommit != readiness.CandidateCommit || binding.Owner.SessionID == "" || binding.Owner.RawRelativePath != "capture/"+binding.Owner.SessionID+"/raw.jsonl" || binding.Owner.HarnessRelativePath != "capture/"+binding.Owner.SessionID+"/harness-evidence.json" || binding.Owner.SealRelativePath != "capture/"+binding.Owner.SessionID+"/capture-seal.json" {
		binding.Failure = "capture_owner_failure"
		return binding, raw
	}
	lineagePath := filepath.Join(root, "capture", binding.Owner.SessionID, "capture_lineage_v3.json")
	...
	if seal.SchemaVersion != "public_match_rehearsal_capture_seal.v1" || seal.CaptureOwnerSHA256 != binding.OwnerSHA256 || seal.RawSHA256 != raw.Manifest.RawSHA256 || seal.RawBytes != raw.Manifest.RawBytes || seal.HarnessSHA256 != binding.HarnessSHA256 || binding.Harness.SchemaVersion != "public_match_rehearsal_harness_evidence.v1" || binding.Harness.SessionID != binding.Owner.SessionID || binding.Harness.RawSHA256 != raw.Manifest.RawSHA256 {
	...
	binding.Failure = validateHarnessEvidence(binding.Harness, raw.Manifest)
	return binding, raw
}
...
func validateHarnessEvidence(value RehearsalHarnessEvidenceV1, manifest RehearsalRawManifestV1) string {
	...
	for index, observation := range value.Process {
		if observation.SourceSequence != uint64(index+1) || !validSHA256(observation.ExecutableSHA256) || observation.StartTicks == 0 || observation.State != "running" {
			return "process_evidence_failure"
		}
		if index == 0 {
			executable, start = observation.ExecutableSHA256, observation.StartTicks
		} else if observation.ExecutableSHA256 != executable || observation.StartTicks != start {
	...
	actions := []string{"preview", "approve", "reject", "emergency_hide"}
...
func buildRehearsalAttempt(repo string, readiness RehearsalReadinessV1, request RehearsalAttemptRequestV1, raw scannedRehearsalRaw, captureBinding rehearsalCaptureBinding) (builtRehearsalAttempt, error) {
	...
	attemptRootPayload, _ := canonical(struct {
		Domain, AcceptedSpec, CandidateCommit, PreflightIndexSHA256, RequestSHA256, CaptureOwnerSHA256, CaptureSealSHA256, HarnessEvidenceSHA256, RawManifestSHA256 string
	}{rehearsalRootDomain, AcceptedRehearsalSpec, readiness.CandidateCommit, readiness.EvidenceIndexSHA256, requestSHA, captureBinding.OwnerSHA256, captureBinding.SealSHA256, captureBinding.HarnessSHA256, manifestSHA})
