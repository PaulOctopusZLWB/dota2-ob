...
func TestRecoveryExecutableAnchorRejectsCoherentResealAndOmission(t *testing.T) {
	setup := func(t *testing.T) (string, string, RehearsalProducerEvidenceV1) {
		t.Helper()
		root := t.TempDir()
		...
		if err := os.WriteFile(binary, []byte("exact-candidate-binary"), 0o700); err != nil {
			t.Fatal(err)
		}
		hash, size, err := fileSHA(binary)
		if err != nil {
			t.Fatal(err)
		}
		identity := RehearsalOwnedProcessIdentityV1{SchemaVersion: "rehearsal_owned_process_identity.v1", PID: 101, Comm: "dota2-ob", ExecutablePath: binary, ExecutablePathSHA256: stringsOf('a'), ExecutableSHA256: hash, ProcessStartTicks: 22, ExecutableDevice: 3, ExecutableInode: 4}
		evidence := RehearsalProducerEvidenceV1{RecoveryPID: identity.PID, RecoveryProcess: []RehearsalOwnedProcessObservationV1{{Boundary: "recovery_start", Identity: identity}, {Boundary: "recovery_terminal", Identity: identity}}, Artifacts: []RehearsalProducerArtifactV1{{Role: "product_binary", Path: "application/dota2-ob", SHA256: hash, Bytes: size}}}
		return root, hash, evidence
	}
	t.Run("valid_exact_candidate_anchor", func(t *testing.T) {
		root, expected, evidence := setup(t)
		if err := validateRecoveryExecutableAnchor(root, evidence, expected); err != nil {
			t.Fatal(err)
		}
	})
	t.Run("coherent_pid_identity_binary_manifest_and_seal_substitution", func(t *testing.T) {
		root, expected, evidence := setup(t)
		replacement := filepath.Join(root, "application/dota2-ob")
		if err := os.WriteFile(replacement, []byte("different-valid-looking-binary"), 0o700); err != nil {
			t.Fatal(err)
		}
		replacementHash, replacementSize, err := fileSHA(replacement)
		if err != nil {
			t.Fatal(err)
		}
		identity := evidence.RecoveryProcess[0].Identity
		identity.PID = 202
		identity.ExecutablePath = "/coherently/substituted/dota2-ob"
		identity.ExecutablePathSHA256 = stringsOf('c')
		identity.ExecutableSHA256 = replacementHash
		identity.ProcessStartTicks = 44
		identity.ExecutableDevice = 5
		identity.ExecutableInode = 6
		evidence.RecoveryPID = identity.PID
		evidence.RecoveryProcess = []RehearsalOwnedProcessObservationV1{{Boundary: "recovery_start", Identity: identity}, {Boundary: "recovery_terminal", Identity: identity}}
		evidence.Artifacts[0].SHA256 = replacementHash
		evidence.Artifacts[0].Bytes = replacementSize
		evidence.ContentSHA256, _ = producerEvidenceContentID(evidence)
		if !validLowerSHA256(evidence.ContentSHA256) {
			t.Fatal("coherent substitute did not reseal")
		}
		if err := validateRecoveryExecutableAnchor(root, evidence, expected); err == nil {
			t.Fatal("coherently resealed recovery identity and binary accepted")
		}
	})
	t.Run("omitted_producer_recovery_cannot_be_retrofitted", func(t *testing.T) {
		root, expected, evidence := setup(t)
		evidence.RecoveryPID = 0
		evidence.RecoveryProcess = nil
		evidence.ContentSHA256, _ = producerEvidenceContentID(evidence)
		if err := validateRecoveryExecutableAnchor(root, evidence, expected); err == nil {
			t.Fatal("omitted producer-time recovery accepted")
		}
	})
}

func TestExactCandidateExecutableBuildIsDeterministic(t *testing.T) {
	repo, _ := os.Getwd()
	repo = filepath.Clean(filepath.Join(repo, "../.."))
	head, err := runText(context.Background(), repo, "git", "rev-parse", "HEAD")
	if err != nil {
		t.Fatal(err)
	}
	firstPath, firstHash, firstCleanup, err := buildExactCandidateExecutable(context.Background(), repo, head)
	if err != nil {
		t.Fatal(err)
	}
	defer firstCleanup()
	secondPath, secondHash, secondCleanup, err := buildExactCandidateExecutable(context.Background(), repo, head)
	if err != nil {
		t.Fatal(err)
	}
	defer secondCleanup()
	if firstHash != secondHash || !validLowerSHA256(firstHash) {
		t.Fatalf("independent exact-candidate builds differ: %s %s", firstHash, secondHash)
	}
	...
	if err != nil || !bytes.Equal(firstBytes, secondBytes) {
		t.Fatal("independent exact-candidate executable bytes differ")
	}
}
