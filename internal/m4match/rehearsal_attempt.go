...
func RehearsalAttempt(ctx context.Context, config RehearsalAttemptConfig) (RehearsalAttemptResultV1, error) {
	...
	if terminal.FailureCode == "" {
		if validateProducerCompletionArtifacts(lease.abs, preflight.SessionID) != nil {
			terminal.FailureCode = "operational_evidence_incomplete"
		} else {
...
func validateProducerCompletionArtifacts(root, sessionID string) error {
	required := []string{"capture-seal.json", "operator-evidence.json", "obs-finalization.json", "resource-samples.json", "recovery-proof.json", "reconciliation.json"}
	for _, name := range required {
		path := filepath.Join(root, "rehearsal", "producer", name)
		info, err := os.Lstat(path)
		if err != nil || !info.Mode().IsRegular() || info.Size() <= 0 || info.Size() > 1<<20 {
			return errors.New("producer completion artifact unavailable")
		}
		payload, err := rootReadFile(path)
		if err != nil || !json.Valid(payload) || !containsSessionID(payload, sessionID) {
			return errors.New("producer completion artifact invalid")
		}
	}
	return nil
}

func containsSessionID(payload []byte, sessionID string) bool {
	var value map[string]any
	if json.Unmarshal(payload, &value) != nil {
		return false
	}
	got, _ := value["session_id"].(string)
