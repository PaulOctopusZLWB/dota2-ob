...
func RehearsalAttempt(ctx context.Context, config RehearsalAttemptConfig) (RehearsalAttemptResultV1, error) {
	...
	terminal.Process = append([]RehearsalProcessObservationV1(nil), producerEvidence.DotaContinuity...)
	rawPath := filepath.Join(lease.abs, "data", "sessions", preflight.SessionID, "raw.jsonl")
	attested, admissionErr := session.ReadAttestedRawV1(rawPath, preflight.SessionID, MaxRehearsalRawBytes, MaxRehearsalRawLineBytes, MaxRehearsalRawRecords)
	terminal.RawAdmission = attested.Receipt
	terminal.RawManifestSHA256, _ = contracts.CanonicalSHA256(attested.Records)
	if continuityErr := validateProducerDotaContinuity(producerEvidence.DotaContinuity, bound, attested.Records, len(attested.Records) > 0); continuityErr != nil {
		if terminal.RawAdmission.Failure.ConcurrentChange == "" {
			terminal.RawAdmission.Failure.ConcurrentChange = "producer_dota_continuity_invalid"
		}
		admissionErr = continuityErr
	}
	if admissionErr != nil {
