...
func VerifyRehearsal(ctx context.Context, dataRoot, repoRoot string) (RehearsalAttemptResultV1, error) {
	...
	if terminal.Outcome == "rehearsal_completed" {
		...
		if err := verifyProducerEvidenceFromRetainedInputs(ctx, lease.abs, repo, preflight, retained); err != nil {
...
func verifyProducerEvidenceFromRetainedInputs(ctx context.Context, root, repo string, preflight RehearsalPreflightV1, evidence RehearsalProducerEvidenceV1) error {
	...
	defer removeExpectedBuild()
	if err := validateRecoveryExecutableAnchor(root, evidence, expectedExecutableSHA256); err != nil {
		return err
	}
	...
	regeneratedProof, regeneratedProcess, regenerationErr := reconstructRecoveryFromRetained(ctx, root, evidence.SessionID, expectedBinary, expectedExecutableSHA256)
	if regenerationErr != nil || len(regeneratedProcess) != 2 || validateRecoveryExecutableIdentity(regeneratedProcess[0].Identity.PID, regeneratedProcess, expectedExecutableSHA256) != nil {
...
func buildExactCandidateExecutable(ctx context.Context, repo, candidateCommit string) (string, string, func(), error) {
	...
	archiveCommand := exec.CommandContext(ctx, "git", "archive", "--format=tar", "--output", archive, candidateCommit)
	archiveCommand.Dir = repo
	if err := archiveCommand.Run(); err != nil {
		cleanup()
		return "", "", func() {}, err
	}
	extractCommand := exec.CommandContext(ctx, "tar", "-xf", archive, "-C", source)
	...
	hash, _, err := fileSHA(binary)
	if err != nil {
		cleanup()
		return "", "", func() {}, err
	}
	return binary, hash, cleanup, nil
}

func validLowerSHA1(value string) bool {
	decoded, err := hex.DecodeString(value)
	return err == nil && len(decoded) == 20 && value == strings.ToLower(value)
}

func validateRecoveryExecutableAnchor(root string, evidence RehearsalProducerEvidenceV1, expectedSHA256 string) error {
	if !validLowerSHA256(expectedSHA256) {
		return errors.New("independent recovery executable identity invalid")
	}
	if err := validateRecoveryExecutableIdentity(evidence.RecoveryPID, evidence.RecoveryProcess, expectedSHA256); err != nil {
		return err
	}
	retainedHash, retainedBytes, retainedErr := fileSHA(filepath.Join(root, "application/dota2-ob"))
	if retainedErr != nil || retainedHash != expectedSHA256 {
		return errors.New("retained application binary differs from exact-candidate build")
	}
	for _, artifact := range evidence.Artifacts {
		if artifact.Role == "product_binary" {
			if artifact.Path != "application/dota2-ob" || artifact.SHA256 != expectedSHA256 || artifact.Bytes != retainedBytes {
				return errors.New("producer application artifact differs from exact-candidate build")
			}
			return nil
		}
	}
	return errors.New("producer application artifact absent")
}

func validateRecoveryExecutableIdentity(pid int, observations []RehearsalOwnedProcessObservationV1, expectedSHA256 string) error {
	if err := validateRecoveryProcessPopulation(pid, observations); err != nil {
		return err
	}
	for _, observation := range observations {
		if observation.Identity.Comm != "dota2-ob" || observation.Identity.ExecutableSHA256 != expectedSHA256 {
			return errors.New("recovery process is not the independent exact-candidate executable")
		}
	}
	return nil
}
...
func reconstructRecoveryFromRetained(ctx context.Context, retainedRoot, sessionID, expectedBinary, expectedSHA256 string) (recoveryProof, []RehearsalOwnedProcessObservationV1, error) {
	...
	if err := os.MkdirAll(filepath.Join(fresh, "application"), 0o700); err != nil {
		return recoveryProof{}, nil, err
	}
	if err := copyFile(expectedBinary, filepath.Join(fresh, "application/dota2-ob"), 0o700); err != nil {
		return recoveryProof{}, nil, err
	}
	freshHash, _, hashErr := fileSHA(filepath.Join(fresh, "application/dota2-ob"))
	if hashErr != nil || freshHash != expectedSHA256 {
		return recoveryProof{}, nil, errors.New("fresh verifier-owned executable identity mismatch")
	}
	...
	if err == nil && (len(process) != 2 || validateRecoveryExecutableIdentity(process[0].Identity.PID, process, expectedSHA256) != nil) {
		err = errors.New("fresh recovery process executable identity mismatch")
	}
