...
func VerifyRehearsal(ctx context.Context, dataRoot, repoRoot string) (RehearsalAttemptResultV1, error) {
	...
	if terminal.Outcome == "rehearsal_completed" {
		if err := validateProducerCompletionArtifacts(lease.abs, terminal.SessionID); err != nil {
...
func validateRehearsalTerminal(value RehearsalTerminalV1) error {
	...
	if value.Outcome == "rehearsal_completed" {
		if value.FailureCode != "" || value.Coverage.State != "complete" || value.Coverage.Delta == nil || value.Coverage.AcceptedFrames == 0 || value.RawAdmission.Failure.Primary != "" || value.RawAdmission.Failure.ConcurrentChange != "" {
