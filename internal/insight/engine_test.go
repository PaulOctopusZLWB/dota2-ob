...
func TestEvaluateLiveOnlyEmitsNoHistoryFamilies(t *testing.T) {
	...
	for index, family := range contracts.HistoricalDisabledFamiliesV1() {
		...
		if execution.Family != family || execution.EligibilitySite != family+".history_unavailable.v1" || execution.Reason != "historical_unavailable" || execution.Evidence.Sequence != observation.Evidence.Sequence || execution.Evidence.RawPayloadSHA256 != observation.Evidence.RawPayloadSHA256 {
